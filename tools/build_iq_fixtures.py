...
rather than through ffmpeg, shifted so the wanted carrier sits at 0 Hz, and
resampled to 48 kHz in the frequency domain (see read_wav_iq, centre_carrier and
resample_to_fixture_rate).
...
guaranteed bit-identical across ffmpeg or numpy versions; re-run the DECODE_IQ_*
tests after regenerating. The synthetic fixtures (--derived-only) are seeded and
reproduce byte for byte on one numpy version.
...
SOURCES = {
    ...
    # and two NFM voice channels with sub-audible signalling (s16, 39.0625 kHz).
    "am_iq_zip": "https://www.sigidwiki.com/images/1/1e/AM_IQ.zip",
    "ctcss_iq_zip": "https://www.sigidwiki.com/images/5/55/IQ_CTCSS_example_482768kHz_IQ.zip",
    "nfm_squelch_iq_zip": "https://www.sigidwiki.com/images/6/6b/Unknown_NFM_squelch_IQ.zip",
}
...
SHA256SUMS = {
    ...
    "ctcss_iq_zip": "1a565be2280d42fd4751b2582aa65530c8e66118126461d0c0a2157ab75ce977",
    "nfm_squelch_iq_zip": "0cb286e7a351854704a5494c0818a304293e9cec793739191ab68243617210a4",
}
...
ZIP_MEMBERS = {
    ...
    "am_iq_zip": {"am_airband_iq": "AM_IQ.wav"},
    "ctcss_iq_zip": {"ctcss_example_iq": "IQ CTCSS example 482768kHz_IQ.wav"},
    "nfm_squelch_iq_zip": {"nfm_squelch_a_iq": "855111kHz_IQ.wav", "nfm_squelch_b_iq": "855361kHz_IQ.wav"},
}
...
#
# Each excerpt is cut from a stretch with a steady carrier and speech, then shifted by
# the centring offset so the wanted carrier sits at 0 Hz -- the analog monitor tunes
# the centre of the capture, and none of these recordings has its carrier there. The
# offset is measured over the excerpt, not nominal: for FM it is the mean instantaneous
# frequency (speech and CTCSS average out; sub-audible data does not quite, so the two
# squelch captures carry a few tens of Hz of it), and for AM it is the phase slope of
# the carrier line. u8 samples are centred on 127.5, the RTL-SDR convention; the AM
# recording's I and Q means are 127.52 and 127.54, so the conventional (x - 128) / 128
# would leave a DC line beside the carrier.
#
#   am_airband_real  AM airband voice, continuous carrier from 22 s to 30 s. The carrier
#                    drifts about +0.5 Hz/s (7.0 to 10.4 Hz), so +/-1.7 Hz remains.
#   nfm_ctcss_real   The channel 12.255 kHz above the recording centre (CTCSS near
...
#
# name, source member, start_s, duration_s, centring offset Hz
ANALOG_EXCERPTS = [
...
# Synthetic NFM (issue #518), deterministic from the seed: a steady 1 kHz test tone at
# 3 kHz peak deviation, the reference the analog replay host scores tone SNR against,
# plus complex Gaussian noise 30 dB below the carrier across the whole 48 kHz span.
# nfm_adjacent_synth adds an unmodulated carrier 12.5 kHz up at -6 dB -- the next
# 12.5 kHz channel -- whose beat the replay host's probe measures at 12.5 kHz. It sits
# outside the default 16 kHz channel filter's passband and inside a 25 kHz one.
#
# name, seed, interferer offset Hz (None = none), interferer level dB re carrier
NFM_SYNTH = [
    ("nfm_tone_synth", 5181, None, None),
    ("nfm_adjacent_synth", 5182, 12500.0, -6.0),
]
NFM_SYNTH_DURATION_S = 1.5
NFM_SYNTH_TONE_HZ = 1000.0
...
def read_wav_iq(path, start_s, duration_s, pad_s):
    """Read an I/Q excerpt from a two-channel u8 or s16 PCM WAV, sample-exact.

    Up to pad_s of real signal is read either side of the excerpt where the file has
    it. Returns (samples, rate_hz, lead): lead is how many samples precede the
    excerpt in the returned array.
    """
    with wave.open(path, "rb") as wav:
        if wav.getnchannels() != 2:
            raise ValueError(f"{path}: expected two channels (I and Q), got {wav.getnchannels()}")
        rate_hz = wav.getframerate()
        width = wav.getsampwidth()
        total = wav.getnframes()
        body_start = int(round(start_s * rate_hz))
        body_len = int(round(duration_s * rate_hz))
        if body_start + body_len > total:
            raise ValueError(f"{path}: excerpt {start_s}+{duration_s} s runs past the {total / rate_hz:.2f} s recording")
        pad = int(round(pad_s * rate_hz))
        first = max(0, body_start - pad)
        last = min(total, body_start + body_len + pad)
        wav.setpos(first)
        raw = wav.readframes(last - first)
    if width == 1:
        values = (np.frombuffer(raw, dtype=np.uint8).astype(np.float64) - 127.5) / 127.5
    elif width == 2:
        values = np.frombuffer(raw, dtype="<i2").astype(np.float64) / 32768.0
    else:
        raise ValueError(f"{path}: unsupported sample width {width} bytes")
    return values[0::2] + 1j * values[1::2], rate_hz, body_start - first


def centre_carrier(samples, rate_hz, offset_hz):
    """Shift the spectrum down by offset_hz so a carrier at offset_hz lands on 0 Hz."""
    n = np.arange(len(samples))
    return samples * np.exp(-2j * math.pi * offset_hz * n / rate_hz)


def resample_to_fixture_rate(samples, rate_hz):
    """Resample complex baseband to SAMPLE_RATE_HZ by truncating or zero-padding its spectrum.

    That is an ideal low-pass at the smaller Nyquist, so a 156.25 kHz recording loses
    everything beyond +/-24 kHz without aliasing it in. The input is zero-extended to a
    whole number of resampling periods; the caller's padding keeps the circular edges
    away from the samples it keeps.
    """
    common = math.gcd(int(rate_hz), SAMPLE_RATE_HZ)
    up = SAMPLE_RATE_HZ // common
    down = int(rate_hz) // common
    n_in = -(-len(samples) // down) * down
    padded = np.zeros(n_in, dtype=np.complex128)
    padded[: len(samples)] = samples
    n_out = n_in // down * up
    spectrum = np.fft.fft(padded)
    half = min(n_in, n_out) // 2
    out = np.zeros(n_out, dtype=np.complex128)
    out[:half] = spectrum[:half]
    out[n_out - half :] = spectrum[n_in - half :]
    return np.fft.ifft(out) * (n_out / n_in)


def build_analog_excerpt(out_dir, name, path, start_s, duration_s, offset_hz):
    """Cut, centre and resample one off-air analog excerpt, then write it as a fixture."""
    samples, rate_hz, lead = read_wav_iq(path, start_s, duration_s, ANALOG_EXCERPT_PAD_S)
    resampled = resample_to_fixture_rate(centre_carrier(samples, rate_hz, offset_hz), rate_hz)
    first = int(round(lead * SAMPLE_RATE_HZ / rate_hz))
    count = int(round(duration_s * SAMPLE_RATE_HZ))
    return write_fixture(out_dir, name, resampled[first : first + count])


def nfm_synth_samples(seed, interferer_hz, interferer_db):
    """FM-modulate the test tone, then add the optional adjacent carrier and the noise floor."""
    count = int(round(NFM_SYNTH_DURATION_S * SAMPLE_RATE_HZ))
    t = np.arange(count) / SAMPLE_RATE_HZ
    beta = NFM_SYNTH_DEVIATION_HZ / NFM_SYNTH_TONE_HZ
    samples = np.exp(1j * beta * np.sin(2.0 * math.pi * NFM_SYNTH_TONE_HZ * t))
    ...
    sigma = math.sqrt(10.0 ** (NFM_SYNTH_NOISE_DB / 10.0) / 2.0)
    return samples + rng.normal(0.0, sigma, count) + 1j * rng.normal(0.0, sigma, count)


def build_nfm_synth(out_dir):
    """Write the deterministic NFM test-tone fixtures (issue #518)."""
    total = 0
...
def resolve_sources(args, needed):
    """Download (or find in the cache) every source in needed, returning {source key: path}."""
    resolved = {}
...
def build_upstream(args):
    """Regenerate the fixtures cut from upstream recordings (network, and ffmpeg for most)."""
    os.makedirs(args.cache, exist_ok=True)

    selected = [entry for entry in FIXTURES if not args.only or entry[0] in args.only]
    excerpts = [entry for entry in ANALOG_EXCERPTS if not args.only or entry[0] in args.only]
    needed = {entry[1] for entry in selected} | {entry[1] for entry in excerpts}
    resolved = resolve_sources(args, needed)
    ...
    for name, source_key, start_s, duration_s, offset_hz in excerpts:
        written = build_analog_excerpt(args.out, name, resolved[source_key], start_s, duration_s, offset_hz)
        total += written
        print(f"{name:28s} rawiq   {written // 1024:6d} KiB")
    return total
