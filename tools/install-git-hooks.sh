...
echo "Tip: checks run in concurrent lanes sized to the machine; DSD_HOOK_JOBS=N caps the workers, DSD_HOOK_SERIAL=1 runs them one at a time."
echo "Tip: set DSD_HOOK_RUN_SCAN_BUILD=1 for the heavier scan-build pass (incremental; DSD_HOOK_SCAN_BUILD_FRESH=1 for a clean rebuild)."
