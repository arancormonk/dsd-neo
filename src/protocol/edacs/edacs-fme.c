...
int
getAfsStringLength(dsd_state* state) {
...
getAfsString(dsd_state* state, char* buffer, int a, int f, int s) {
    if (!isCustomAfsString(state)) {
        ...
        snprintf(buffer, 6 + 1, "%02d-%02d%01d", a, f, s);
        return 6;
    }
...
//RTL Input
        ...
        if (state->ea_mode == 0) {
            ...
            if (afs == -1) {
                fprintf(stderr, " TGT [ SYSTEM ] LCN [%02d] All-Call", lcn);
            } else {
                fprintf(stderr, " TGT [%08d] LCN [%02d]", afs, lcn);
            }
        }
    ...
    */
        ...
        if (state->ea_mode == 1) {
            ...
            if (mt1 == 0x1F) {
                ...
                if (mt2 == 0x0) {
                    ...
                    int cc_lcn =
                        (msg_1 & 0x3E000)
                        >> 13; //shifted to allow this example to be CC LCN 1, as was reported at the time of capture
                ...
                //Adjacent Sites
                    ...
                    if (adj_site > 0) {
                        fprintf(stderr, " :: Site ID [%02X][%03d] Index [%d] on CC LCN [%02d]%s", adj_site, adj_site,
                                adj_idx, adj_lcn, getLcnStatusString(lcn));
                    } else {
                        fprintf(stderr, " :: Total Indexed [%d]",
                                adj_idx); //if site value is 0, then this tells us the total number of adjacent sites
                    }

                    if (adj_site == 0 && adj_idx == 0) {
                        fprintf(stderr, " [Adjacency Table Reset]");
                    } else if (adj_site != 0 && adj_idx == 0) {
                        fprintf(stderr, " [Priority System Definition]");
                    } else if (adj_site == 0 && adj_idx != 0) {
                        fprintf(stderr, " [Adjacencies Table Length Definition]");
                    } else {
                        fprintf(stderr, " [Adjacent System Definition]");
                    }
                    fprintf(stderr, "%s", KNRM);
                }
                //Status/Message
                    ...
                    if (status == 248) {
                        fprintf(stderr, " Status Request :: Target [%08d]", source);
                    } else {
                        fprintf(stderr, " Message Acknowledgement :: Status [%03d] Source [%08d]", status, source);
                    }
                    fprintf(stderr, "%s", KNRM);
                }
                //Unit Enable / Disable
                    ...
                    if (qualifier == 0x0) {
                        fprintf(stderr, " [Temporary Disable]");
                    } else if (qualifier == 0x1) {
                        fprintf(stderr, " [Corrupt Personality]");
                    } else if (qualifier == 0x2) {
                        fprintf(stderr, " [Revoke Logical ID]");
                    } else {
                        fprintf(stderr, " [Re-enable Unit]");
                    }
                ...
                //Control Channel LCN
                    ...
                    if (lcn != 0) {
                        ...
                        if (state->edacs_cc_lcn > state->edacs_lcn_count
                            && lcn < 26) //26, or 27. shouldn't matter don't think cc lcn will give a status lcn val
                        {
                            state->edacs_lcn_count = state->edacs_cc_lcn;
                        }
                        fprintf(stderr, " :: System ID [%04X] CC LCN [%02d]%s", system, state->edacs_cc_lcn,
                ...
                //Site ID
                    ...
                    fprintf(stderr, "%s", KYEL);
                    fprintf(stderr, " Extended Addressing :: Site ID [%02llX][%03lld] Area [%02X][%03d]", site_id,
                            site_id, area, area);
                    fprintf(stderr, "%s", KNRM);
                ...
                //Patch Groups / Dynamic Regroup -- Reverse Engineer WIP from observations, guesswork, and documented P25 MFID A4 regroup operations
                    ...
                    int unk2 =
                        (msg_2 & 0x700000) >> 20;   //unknown 3-bit value, possibly linked to TGA when patch is deleted
                    int target = (msg_2 & 0xFFFFF); //target group or individual ID (20-bit) to include in supergroup
                    ...
                    if (sgid != target) {
                        ...
                        fprintf(
                            stderr, " OPT: %01X;",
                            tga); //this value seems to always be 7 for an active patch, 0 for a termination of a patch (6 if assigned the unk2 value)
                        if (unk1) {
                        ...
                        fprintf(
                            stderr, " SSN: %02X;",
                            ssn); //this may or may not be a unique value to each SGID, is 1F for termination of a patch
                    }
            ...
            //TDMA Group Grant Update (never observed, unknown if ever used on any EDACS system)
                ...
                fprintf(stderr, "%s", KGRN);
                fprintf(stderr, " TDMA Group Call :: Group [%05d] Source [%08d] LCN [%02d]%s", group, source, lcn,
                        getLcnStatusString(lcn));
                fprintf(stderr, "%s", KNRM);
            }
            //Data Group Grant Update
                ...
                fprintf(stderr, "%s", KBLU);
                fprintf(stderr, " Data Group Call :: Group [%05d] Source [%08d] LCN [%02d]%s", group, source, lcn,
                        getLcnStatusString(lcn));
                fprintf(stderr, "%s", KNRM);
            }
            ...
            // - 0x06 analog group voice
                ...
                fprintf(stderr, "%s", KGRN);
                if (is_digital == 0) {
                    fprintf(stderr, " Analog Group Call");
                } else {
                    fprintf(stderr, " Digital Group Call");
                }
                if (is_update == 0) {
                ...
                fprintf(stderr, " :: Group [%05d] Source [%08d] LCN [%02d]%s", group, source, lcn,
                        getLcnStatusString(lcn));
                ...
                if (is_tx_trunking == 0) {
                    fprintf(stderr, " [Message Trunking]");
                }
                if (is_emergency == 1) {
                    fprintf(stderr, "%s", KRED);
            ...
            //I-Call/Test Call Assignment/Update
                ...
                if (target == 0 && source == 0) {
                    ...
                    if (is_digital == 0) {
                        fprintf(stderr, " Analog I-Call");
                    } else {
                        fprintf(stderr, " Digital I-Call");
                    }
                    if (is_update == 0) {
                        fprintf(stderr, " Assignment");
                    } else {
                    ...
                    fprintf(stderr, " :: Target [%08d] Source [%08d] LCN [%02d]%s", target, source, lcn,
                            getLcnStatusString(lcn));
                }
            ...
            //Channel assignment (unknown reason, just know it assigns an LCN in the expected order; believed related to data)
                ...
                fprintf(stderr, "%s", KBLU);
                fprintf(stderr, " Channel Assignment (Unknown Data) :: Source [%08d] LCN [%02d]%s", source, lcn,
                        getLcnStatusString(lcn));
                fprintf(stderr, "%s", KNRM);
            ...
            //System All-Call Grant Update
                ...
                fprintf(stderr, "%s", KMAG);
                if (is_digital == 0) {
                    fprintf(stderr, " Analog System All-Call");
                } else {
                    fprintf(stderr, " Digital System All-Call");
                }
                if (is_update == 0) {
                    fprintf(stderr, " Assignment");
                } else {
        ...
        //Start Standard or Networked Mode
            ...
            if (mt_a == 0x0 || mt_a == 0x1 || mt_a == 0x2 || mt_a == 0x3) {
                ...
                if (is_agency_call == 1) {
                    fprintf(stderr, " [Agency]");
                } else if (is_agency_call == 1) {
                    fprintf(stderr, " [Fleet]");
                }
                if (is_tx_trunk == 0) {
                    fprintf(stderr, " [Message Trunking]");
                }
            ...
            //Data Call Channel Assignment (6.2.4.3)
                ...
                if (is_individual_id == 1) {
                    ...
                    fprintf(stderr, " Group [%04d]", target);
                }
                if (is_from_lid == 1) {
                    fprintf(stderr, " -->");
                } else {
            ...
            //Use MT-B
                ...
                if (mt_b == 0x0) {
                    ...
                    fprintf(stderr, "%s", KBLU);
                    if (status == 248) {
                        fprintf(stderr, " Status Request :: LID [%05d]", lid);
                    } else {
                        fprintf(stderr, " Message Acknowledgement :: Status [%03d] LID [%05d]", status, lid);
                    }
                    fprintf(stderr, "%s", KNRM);
                }
                ...
                //Test calls reverse engineered from HartLink system
                    ...
                    if (is_individual == 0) {
                        ...
                        fprintf(stderr, " Callee [%05d] Caller [%05d]", target, source);
                    }
                    fprintf(stderr, " LCN [%02d]%s", lcn, getLcnStatusString(lcn));
                    if (is_agency_call == 1) {
                        fprintf(stderr, " [Agency]");
                ...
                //Use MT-D
                    ...
                    //Adjacent Site Control Channel (6.2.4.12)
                        ...
                        fprintf(stderr, "%s", KYEL);
                        fprintf(stderr,
                                " Adjacent Site Control Channel :: Site ID [%02X][%03d] Index [%1d] LCN [%02d]%s",
                                adj_site_id, adj_site_id, adj_site_index, adj_cc_lcn, getLcnStatusString(adj_cc_lcn));
                        if (adj_site_id == 0 && adj_site_index == 0) {
                            fprintf(stderr, " [Adjacency Table Reset]");
                        } else if (adj_site_id != 0 && adj_site_index == 0) {
                            fprintf(stderr, " [Priority System Definition]");
                        } else if (adj_site_id == 0 && adj_site_index != 0) {
                            fprintf(stderr, " [Adjacencies Table Length Definition]");
                        } else {
                    ...
                    //Assignment to Auxiliary Control Channel (6.2.4.15)
                        ...
                        fprintf(stderr, "%s", KYEL);
                        fprintf(stderr, " Assignment to Auxiliary CC :: Group [%04d] Aux CC LCN [%02d]%s", group,
                                aux_cc_lcn, getLcnStatusString(aux_cc_lcn));
                        fprintf(stderr, "%s", KNRM);
                    }
                    ...
                    //Site ID (6.2.4.18)
                        ...
                        fprintf(stderr, "%s", KYEL);
                        fprintf(stderr, " Standard/Networked :: Site ID [%02X][%03d] Priority [%1d] CC LCN [%02d]%s",
                                site_id, site_id, priority, cc_lcn, getLcnStatusString(cc_lcn));
                        if (is_failsoft == 1) {
                            fprintf(stderr, "%s", KRED);
                    ...
                    //Dynamic Regrouping (6.2.4.20)
                        ...
                        fprintf(stderr, "%s", KYEL);
                        fprintf(stderr,
                                " Dynamic Regrouping :: Plan [%02d] Knob position [%1d] LID [%05d] Group [%04d] Fleet "
                                "bits [%1d]",
                                plan, knob + 1, lid, group, fleet_bits);
                        if (type == 0) {
                            fprintf(stderr, " [Forced select, no deselect]");
                        } else if (type == 1) {
                            fprintf(stderr, " [Forced select, optional deselect]");
                        } else if (type == 2) {
                            fprintf(stderr, " [Reserved]");
                        } else {
                            fprintf(stderr, " [Optional select]");
                        }
                        fprintf(stderr, "%s", KNRM);
                    }
        ...
        //let users know they need to select an operational mode with the switches below
            ...
            fprintf(stderr,
                    " Detected EDACS: Use -fh, -fH, -fe, or -fE for std, esk, ea, or ea-esk to specify the type");
            fprintf(stderr, "\n");
            fprintf(stderr, " MSG_1 [%07llX]", msg_1);
            fprintf(stderr, " MSG_2 [%07llX]", msg_2);
        }
