 ...
 */
static void
initialize_symbol_heuristics(SymbolHeuristics* sh) {
    sh->count = 0;
    sh->index = 0;
    sh->sum = 0;
    sh->var_sum = 0;
...
void
initialize_p25_heuristics(P25Heuristics* heuristics) {
    int i, j;
    for (i = 0; i < 4; i++) {
        for (j = 0; j < 4; j++) {
 ...
 */
static void
debug_print_symbol_heuristics(int previous_dibit, int dibit, SymbolHeuristics* sh) {
    float mean, sd;
    int n;
