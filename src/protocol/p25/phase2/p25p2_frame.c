...
p25_p2_frame_reset(void) {
    ...
    memset(p2llr, 0, sizeof(p2llr));
    memset(p2xllr, 0, sizeof(p2xllr));
    ...
    memset(ess_a_llr, 0, sizeof(ess_a_llr));
...
process_4V(dsd_opts* opts, dsd_state* state) {
    ...
    if (state->fourv_counter[state->currentslot] == 0) {
        memset(state->ess_b[state->currentslot], 0, sizeof(state->ess_b[state->currentslot]));
        memset(state->ess_b_llr[state->currentslot], 0, sizeof(state->ess_b_llr[state->currentslot]));
    }
    for (int i = 0; i < 24; i++) {
        int out = i + (state->fourv_counter[state->currentslot] * 24);
        int in = i + 148 + vc_counter;
        state->ess_b[state->currentslot][out] = p2xbit[in];
        state->ess_b_llr[state->currentslot][out] = p2xllr[in];
    }
