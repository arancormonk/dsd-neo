...
dmr_lrrp(dsd_opts* opts, dsd_state* state, uint16_t len, uint32_t source, uint32_t dest, uint8_t* DMR_PDU) {
        ...
        switch (token) {
            ...
            case 0x55: //circle-3d
                if (message_len > 0 && lat == 0) {
                    ...
                    lat = (((DMR_PDU[i + 1] << 24) + (DMR_PDU[i + 2] << 16) + (DMR_PDU[i + 3] << 8) + DMR_PDU[i + 4])
                           * 1);
                    lon = (((DMR_PDU[i + 5] << 24) + (DMR_PDU[i + 6] << 16) + (DMR_PDU[i + 7] << 8) + DMR_PDU[i + 8])
                           * 1);
    ...
    if (message_len > 0) {
        ...
        if (lrrp_confidence >= 3) //minimal of src, dst, and message len indicator
        {
            ...
            if (vel_set) {
                sprintf(velstr, " %.4lf km/h", velocity * 3.6);
            }
            if (deg_set) {
