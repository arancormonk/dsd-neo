    ...
    if (IrrecoverableErrors == 0) {
        ...
        if (MFID == 0x68) //Hytera Enhanced
        {
            if (state->currentslot == 0) {
                ...
                state->payload_mi =
                    ((unsigned long long int)PI_BYTE[3] << 32ULL) | ((unsigned long long int)PI_BYTE[4] << 24)
                    | ((unsigned long long int)PI_BYTE[5] << 16) | ((unsigned long long int)PI_BYTE[6] << 8)
                    | ((unsigned long long int)PI_BYTE[7] << 0);
            } else {
            ...
            fprintf(stderr, " DMR PI H- ALG ID: %02X; KEY ID: %02X; MI(40): %02X%02X%02X%02X%02X;", PI_BYTE[0],
                    PI_BYTE[2], PI_BYTE[3], PI_BYTE[4], PI_BYTE[5], PI_BYTE[6], PI_BYTE[7]);
        ...
        else if (MFID == 0x10) //DMRA
        {
            ...
            if (state->currentslot == 0) {
                ...
                state->payload_keyid = PI_BYTE[2];
                state->payload_mi =
                    ((unsigned long long int)PI_BYTE[3] << 24ULL) | ((unsigned long long int)PI_BYTE[4] << 16ULL)
                    | ((unsigned long long int)PI_BYTE[5] << 8ULL) | ((unsigned long long int)PI_BYTE[6] << 0ULL);
                if (state->payload_algid < 0x26) {
                    fprintf(stderr, "%s ", KYEL);
                    fprintf(stderr, "\n Slot 1");
                    fprintf(stderr, " DMR PI H- ALG ID: %02X; KEY ID: %02X; MI(32): %08llX;", state->payload_algid,
                            state->payload_keyid, state->payload_mi);
                    ...
                    if (state->payload_algid & 0x20) {
                        ...
                        fprintf(stderr, " DMRA Compatible");
                    }
            ...
            if (state->currentslot == 1) {
                ...
                if (state->payload_algidR < 0x26) {
                    fprintf(stderr, "%s ", KYEL);
                    fprintf(stderr, "\n Slot 2");
                    fprintf(stderr, " DMR PI H- ALG ID: %02X; KEY ID: %02X; MI(32): %08llX;", state->payload_algidR,
                            state->payload_keyidR, state->payload_miR);
...
LFSR128d(dsd_state* state) {
    ...
    if (state->currentslot == 1) {
        fprintf(stderr, "%s", KYEL);
        fprintf(stderr, " Slot 2");
        fprintf(stderr, " DMR PI C- ALG ID: %02X; KEY ID: %02X; MI(128): ", state->payload_algidR,
...
hytera_lfsr(uint8_t* mi, uint8_t* taps, uint8_t len) {
