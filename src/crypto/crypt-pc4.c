...
static void
md2_init(PC4Context* ctx) {
    ctx->x1 = 0;
    ctx->x2 = 0;
    for (ctx->i = 0; ctx->i < n1; ctx->i++) {
        ctx->h2[ctx->i] = 0;
    }
    for (ctx->i = 0; ctx->i < n1; ctx->i++) {
        ctx->h1[ctx->i] = 0;
...
/* Finalize MD2-II */
static void
md2_end(PC4Context* ctx, unsigned char h4[n1]) {
    unsigned char h3[n1];
    int i, n4 = n1 - ctx->x2;
    for (i = 0; i < n4; i++) {
...
compute(PC4Context* ctx, uint8_t* tab1, uint8_t round) {
    ...
    ctx->tot[1] = (uint8_t)((ctx->perm[round][tab1[ctx->permut[1][0]]] + ctx->perm[round][tab1[ctx->permut[1][1]]])
                            ^ ctx->perm[round][tab1[ctx->permut[1][2]]]);
    ctx->tot[1] = (uint8_t)(ctx->tot[1] + ctx->new1[ctx->tot[1]]);
    ctx->tot[2] = (uint8_t)((ctx->perm[round][tab1[ctx->permut[2][0]]] + ctx->perm[round][tab1[ctx->permut[2][1]]])
...
void
hexbin(PC4Context* ctx, short* q, uint8_t w, uint8_t hex) { // warning: unused parameter ‘ctx’ [-Wunused-parameter]
    short* bits = (short*)q;
