...
Item {
    ...
    TestCase {
        ...
        function test_monitor_scrim_data() {
            return [{tag: "site-stop", sheet: "siteSheet", target: "stopListeningButton"},
                    {tag: "network-stop", sheet: "networkSheet", target: "stopListeningButton"},
                    {tag: "site-spectrum", sheet: "siteSheet", target: "openSpectrumButton"},
                    {tag: "network-spectrum", sheet: "networkSheet", target: "openSpectrumButton"}];
        }
        ...
        function test_sites_button_under_monitor_sheets_data() {
            return [{tag: "site", sheet: "siteSheet"}, {tag: "network", sheet: "networkSheet"}];
        }
        ...
        function openEdit() {
            ...
            sheet.openRow(1001, 1001, "Dispatch", true, true, 25, false,
                          talkgroups.policyContext, talkgroups.policyGeneration);
            return sheet;
        }
        function test_edit_scrim_data() {
            return [{tag: "card", target: "card"}, {tag: "listen-all", target: "listenAllButton"},
                    {tag: "block-all", target: "noTuneAllButton"}];
        }
        function test_edit_scrim(data) {
            ...
            var target = data.target === "card" ? item("talkgroupGrid").itemAtIndex(0) : item(data.target);
            waitForRendering(sheet);
            var point = target.mapToItem(sheet, target.width / 2, target.height / 2);
            ...
            mouseClick(target, target.width / 2, target.height / 2);
            tryVerify(function() { return testContext.talkgroupListenCalls() + testContext.allTalkgroupsListenCalls() === 1; });
        }
