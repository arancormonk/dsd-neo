...
test_source_alias_services(void) {
    ...
    g_src_load_result = 0;
    g_src_count = 0;
    rc |= expect_int("source zero rows refused", svc_import_src_list(opts, state, "empty.csv"), -1);
    rc |= expect_int("source empty freed", g_src_free_calls, 1);
    rc |= expect_int("source empty skips install", g_src_install_calls, 0);
    rc |= expect_str("source empty preserves path", opts->src_in_file, "first.csv");
    g_src_count = 2;
    opts->trunk_scan_enabled = 1;
    ...
    rc |= expect_int("source installed once", g_src_install_calls, 1);
    rc |= expect_int("source adopts same candidate", g_src_installed == &g_src_candidate, 1);
    rc |= expect_int("source adopted candidate not freed", g_src_free_calls, 1);
