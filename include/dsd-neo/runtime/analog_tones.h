 ...
 *
 * Analog-only decoding with input monitoring and the FM analog kind (the Analog preset, not AM:
 * CTCSS and DCS are FM signalling), on a PCM input (Pulse, stdin, WAV, UDP, TCP) or on an
 * RTL-family stream whose output is monitor audio (asked of the RTL stream-metrics hook, whose
 * default answers monitor audio). Symbol-file and null inputs carry no audio to hear. The -8
 * source monitor during digital decoding and EDACS analog voice are out: neither is analog-only.
 *
