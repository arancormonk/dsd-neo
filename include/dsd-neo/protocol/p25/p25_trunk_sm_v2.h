...
/**
 * @file
 * @brief Simplified unified P25 trunking state machine (v2).
 *
 * This is a streamlined replacement for the dual p25_trunk_sm + p25_p2_sm_min
 ...
 */
void p25_sm_v2_init(p25_sm_ctx_t* ctx, dsd_opts* opts, dsd_state* state);
 ...
 */
void p25_sm_v2_event(p25_sm_ctx_t* ctx, dsd_opts* opts, dsd_state* state, const p25_sm_event_t* ev);
 ...
 * @param state Decoder state.
 */
void p25_sm_v2_tick(p25_sm_ctx_t* ctx, dsd_opts* opts, dsd_state* state);
...
p25_sm_v2_get_state(const p25_sm_ctx_t* ctx) {
 ...
 */
const char* p25_sm_v2_state_name(p25_sm_state_e state);
 ...
 */
p25_sm_ctx_t* p25_sm_v2_get(void);

/**
 * @brief Check if the v2 state machine should be used.
 *
 * Always returns 1 since v2 is now the only implementation.
 *
 * @param opts Decoder options (ignored).
 * @return Always returns 1.
 */
int p25_sm_v2_enabled(dsd_opts* opts);

/**
 * @brief Emit a v2 event if v2 SM is enabled, otherwise no-op.
 *
 * Convenience function for decoders to emit events without checking
 ...
 */
void p25_sm_v2_emit(dsd_opts* opts, dsd_state* state, const p25_sm_event_t* ev);

/**
 * @brief Emit PTT event for a slot.
 */
void p25_sm_v2_emit_ptt(dsd_opts* opts, dsd_state* state, int slot);

/**
 * @brief Emit ACTIVE event for a slot.
 */
void p25_sm_v2_emit_active(dsd_opts* opts, dsd_state* state, int slot);

/**
 * @brief Emit END event for a slot.
 */
void p25_sm_v2_emit_end(dsd_opts* opts, dsd_state* state, int slot);

/**
 * @brief Emit IDLE event for a slot.
 */
void p25_sm_v2_emit_idle(dsd_opts* opts, dsd_state* state, int slot);

/**
 * @brief Emit TDU (P1 terminator) event.
 */
void p25_sm_v2_emit_tdu(dsd_opts* opts, dsd_state* state);
 ...
 */
void p25_sm_v2_emit_enc(dsd_opts* opts, dsd_state* state, int slot, int algid, int keyid, int tg);
 ...
 */
void p25_sm_v2_on_neighbor_update(dsd_opts* opts, dsd_state* state, const long* freqs, int count);
 ...
 */
int p25_sm_v2_next_cc_candidate(dsd_state* state, long* out_freq);
 ...
 */
void p25_sm_v2_release(p25_sm_ctx_t* ctx, dsd_opts* opts, dsd_state* state, const char* reason);
 ...
 */
int p25_sm_v2_audio_allowed(p25_sm_ctx_t* ctx, dsd_state* state, int slot);
 ...
 * @param keyid Key ID.
 */
void p25_sm_v2_update_audio_gate(p25_sm_ctx_t* ctx, dsd_state* state, int slot, int algid, int keyid);
