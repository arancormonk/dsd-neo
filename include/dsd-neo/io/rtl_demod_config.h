 ...
 * allows. An explicit width stays as requested, never clamped or swapped for another design; when the new rate cannot
 * realize it the channel has no width-driven plan, so the caller must not run the stream on it (the RTL stream refuses
 * a retune that lands there, and stops when the device will not leave such a rate). No-op unless the analog monitor
 * output is running on the analog channel (dsd_demod_analog_monitor_active()): CQPSK toggled on under the analog
 * family, or a typed digital scan row's symbol profile, keeps its own profile filter.
 *
 ...
 * analog family (including the M17 encoder). Where a SoapySDR or Airspy device or an I/Q replay forces rate_out, a
 * width it cannot filter is refused with narrowing the width as the fix, not an RTL DSP bandwidth, which does not move
 * that rate (dsd_analog_width_check_forced_rate()).
 *
