...
- Conventional scan mode: `-Y` (not trunking; scans for sync on the row's decoder class or the global decoders). The later of `-T`/`-Y` wins. For NXDN the hold is refreshed
