...
- Do not use wide-character conversions (`%lc`, `%ls`) in format strings. They hand a code unit to the C runtime to encode, and off-air text regularly contains one that has no encoding — a lone surrogate. glibc drops it; the Windows UCRT reports the failed conversion as a string of length -1 and then writes the stack to the stream until the process faults. Decode with `<dsd-neo/core/utf16.h>` and print through `dsd_unicode_fput_scalar()`.
