...
- Compare floating-point test results with an explicit tolerance. The local Semgrep rule covers initialized scalar `float`/`double` values and local floating-point array subscripts compared against identifiers, including const and uninitialized arrays. Explicit literal sentinel checks remain allowed. It is a syntactic guardrail, not complete type analysis of pointers, fields, or return values. Strict scans run its positive and negative fixtures in `semgrep/dsd-neo.cpp` before scanning the tree.
