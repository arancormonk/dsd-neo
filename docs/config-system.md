...
| `persist_tg_lockouts` | BOOL | Save quick `!`/`@` and Qt/Android **Lock out** actions to the configured global groups file; `false` keeps temporary avoids in memory and labels the Qt/Android button **Avoid**. **Skip** is unaffected | `true` |
