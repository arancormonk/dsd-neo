...
| `Carrier` | an analog row's carrier is open (an `nfm` row or `nfm-conventional` target, or any row scanned under `-fA`): squelch open, whether or not audio plays | `-Y`: the hangtime window; trunk scan: the activity hold |
