...
REM Point PulseAudio to bundled module directory if present
set "PULSE_MODDIR="
...
:found_moddir
REM Use POSIX path if available to avoid drive-letter parsing issues
if defined PULSE_MODDIR_POSIX (
...
if not exist "%ROOT%dev\shm" mkdir "%ROOT%dev\shm"
...
if exist "%BIN%\pulseaudio.exe" (
  ...
  if exist "%ETC%\pulse\default.pa" (
    ...
    if defined PULSE_DL_PATH (
      "%BIN%\pulseaudio.exe" -n --daemonize=1 --exit-idle-time=-1 --dl-search-path="%PULSE_DL_PATH%" >nul 2>nul
    ) else (
